...
class TF_Encoder(nn.Module):
    ...
    def __init__(self, pad_word_idx, device = 'cuda'):
        super().__init__()

        self.device = device
        self.pad_word_idx = pad_word_idx
        self.bert_model = AutoModel.from_pretrained('nlpaueb/legal-bert-base-uncased')
        count = 0
    ...
    def forward(self, input_x = []):
        ...
        for doc in input_x:
            ...
            hidden_reps = self.bert_model(sents)
            hidden = hidden_reps[0][:, 0, :200]
            # print(hidden.shape)
...
class CRF(nn.Module):
    def __init__(self, n_tags, sos_tag_idx, eos_tag_idx, pad_tag_idx=None):
        ...
        print(self.n_tags, self.SOS_TAG_IDX, self.EOS_TAG_IDX, self.PAD_TAG_IDX)
        self.init_weights()
    ...
    def _viterbi_decode(self, emissions, mask):
        ...
        last_transition = self.transitions[:, self.EOS_TAG_IDX]
        end_scores = alphas + last_transition.unsqueeze(0)
        # print("Edn scores device is :"+str(end_scores.get_device()))
        # get the final most probable score and the final most probable tag
        max_final_scores, max_final_tags = torch.max(end_scores, dim=1)
        # print("torch max done")

        # find the best sequence of labels for each sample in the batch
        best_sequences = []
        emission_lengths = mask.int().sum(dim=1)
        # print("going for for loop now")
        for i in range(batch_size):
            # print("emission_lengths is on dev : "+str(emission_lengths.get_device()))

            # recover the original sentence length for the i-th sample in the batch
            sample_length = emission_lengths[i].item()

            # print("emission_lengths is on dev, done : "+str(emission_lengths.get_device()))

            # print("max final tag is  : "+str(max_final_tags.get_device()))
