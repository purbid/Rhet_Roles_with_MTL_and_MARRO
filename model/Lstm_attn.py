...
class Attn_BiLSTM(nn.Module):
    def __init__(self, n_tags, sent_emb_dim, sos_tag_idx, eos_tag_idx, pad_tag_idx, vocab_size=0, word_emb_dim=0,
                     pad_word_idx=0, pretrained=False, device='cpu', num_blocks = 4,
                     dropout_rate = 0.2,
        ...
        self.gru = nn.LSTM(sent_emb_dim, sent_emb_dim // 2 , batch_first=True, bidirectional=True)
    ...
    def forward(self, x, y=[]):
        ...
        mask = (y_tensor != 0).bool()
        return  decode_entity(self.emissions.to(device), mask)
    ...
    def _loss(self, y):
        ...
        loss = F.cross_entropy(self.emissions.view(-1, self.emissions.shape[-1]), tensor_y.view(-1), ignore_index=0)
        return loss
