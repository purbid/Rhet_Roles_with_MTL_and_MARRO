...
class Tf_Hier_LSTM_CRF_Classifier(nn.Module):
    def __init__(self, n_tags, sent_emb_dim, sos_tag_idx, eos_tag_idx, pad_tag_idx, vocab_size=0, word_emb_dim=0,
        ...
        self.emitter = LSTM_Emitter(n_tags, 768, 768, device=self.device).to(self.device)
    ...
    def forward(self, x, y =[]):
        ...
        # print(batch_size, seq_lengths, max_seq_len)
        ...
        # print(type(tensor_x),type(tensor_x[0]),type(tensor_x[0][0]))
        ##e list[batch_size, sents_per_doc, sent_emb_dim] --> tensor[batch_size, max_seq_len, sent_emb_dim]
